...
class ProxyHeaderUserAuthenticationMiddleware(django.contrib.auth.middleware.RemoteUserMiddleware):
    ...
    if re.match(r"^http_", proxy_authentication_username, re.I):
        ...
        header = "HTTP_" + re.sub(r"[-_]", "_",
                                  getattr(settings, 'PROXY_HEADER_AUTHENTICATION_HEADERS', {}).get('username',
                                                                                                   '').upper())
    ...
    def authenticate(self, request, remote_user):
        ...
        if user:
            ...
            if user_portfolio is None:
                ...
                user_portfolio = Portfolio(title="{}".format(user.username),
                                           description="Default portfolio of {}".format(user.username))
                user_portfolio.save()
