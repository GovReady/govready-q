...
if os.path.exists(local("environment.json")):
    ...
    environment[
        "secret-key"] = make_secret_key()  # Generate a new key since the initial one was printed for the user for edification
...
if "host" in environment:
    ...
    print(
        "WARNING: Use of 'host' environment parameter deprecated. Please use 'govready-url' environment parameter in future.")
...
REST_FRAMEWORK = {
    'DEFAULT_PAGINATION_CLASS': 'api.utils.pagination.Pagination',
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': (
        "siteapp.authentication.authentication_backend.APITokenBackend",
    )
}
...
MIDDLEWARE = [
    ...
    # 'siteapp.middleware.logging.LoggingMiddleware',
...
SESSION_EXPIRE_AT_BROWSER_CLOSE = environment.get('session_security_expire_at_browser_close', True) if not DEBUG else True
SESSION_SECURITY_WARN_AFTER = environment.get('session_security_warn_after', 1200) if not DEBUG else 85800
SESSION_SECURITY_EXPIRE_AFTER = environment.get('session_security_expire_after', 1800) if not DEBUG else 86400
...
if os.path.join(siteapp_path, STATIC_ROOT) == os.path.join(siteapp_path, "siteapp/static"):
    raise ValidationError(
        'STATIC_ROOT directory for collecting static files should never be set to source of uncollected static files (e.g. `siteapp/static`). Please fix environment `static` setting.')
