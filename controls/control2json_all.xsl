...
usage: $> xsltproc control2json_all.xsl 800-53-controls.xml
example: $>  xsltproc lib/control2json_all.xsl data/800-53-controls.xml
...
<xsl:param name="controlnumber">*</xsl:param>
...
<xsl:template match="controls:controls/controls:control">
    <xsl:variable name="filename" select="concat( c:number, '.md' )"/>"<xsl:value-of select='c:number'/>": { "id": "<xsl:value-of select='c:number'/>",
