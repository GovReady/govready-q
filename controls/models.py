...
class Statement(auto_prefetch.Model, BaseModel):
    sid = models.CharField(max_length=100, help_text="Statement identifier such as OSCAL formatted Control ID",
                           unique=False, blank=True, null=True)
    sid_class = models.CharField(max_length=200,
                                 help_text="Statement identifier 'class' such as 'NIST_SP-800-53_rev4' or other OSCAL catalog name Control ID.",
                                 unique=False, blank=True, null=True)
    pid = models.CharField(max_length=20, help_text="Statement part identifier such as 'h' or 'h.1' or other part key",
                           unique=False, blank=True, null=True)
    ...
    parent = auto_prefetch.ForeignKey('self', help_text="Parent statement", related_name="children",
                                      on_delete=models.SET_NULL, blank=True, null=True)
    prototype = auto_prefetch.ForeignKey('self', help_text="Prototype statement", related_name="instances",
                                         on_delete=models.SET_NULL, blank=True, null=True)
    producer_element = auto_prefetch.ForeignKey('Element', related_name='statements_produced', on_delete=models.CASCADE,
                                                blank=True, null=True,
                                                help_text="The element producing this statement.")
    consumer_element = auto_prefetch.ForeignKey('Element', related_name='statements_consumed', on_delete=models.CASCADE,
                                                blank=True, null=True, help_text="The element the statement is about.")
    mentioned_elements = models.ManyToManyField('Element', related_name='statements_mentioning', blank=True,
    ...
    import_record = auto_prefetch.ForeignKey(ImportRecord, related_name="import_record_statements",
    ...
    def create_instance_from_prototype(self, consumer_element_id):
        ...
        smts_existing = Statement.objects.filter(consumer_element__id=consumer_element_id,
                                                 statement_type="control_implementation").select_related('prototype')
...
class Element(auto_prefetch.Model, TagModelMixin, BaseModel):
    name = models.CharField(max_length=250, help_text="Common name or acronym of the element", unique=True, blank=False,
                            null=False)
    full_name = models.CharField(max_length=250, help_text="Full name of the element", unique=False, blank=True,
                                 null=True)
    ...
    roles = models.ManyToManyField('ElementRole', related_name='elements', blank=True,
                                   help_text="Roles assigned to the Element")
    uuid = models.UUIDField(default=uuid.uuid4, editable=True,
                            help_text="A UUID (a unique identifier) for this Element.")
    import_record = auto_prefetch.ForeignKey(ImportRecord, related_name="import_record_elements",
                                             on_delete=models.CASCADE,
                                             unique=False, blank=True, null=True,
    ...
    def get_control_impl_smts_prototype_count(self):
        ...
        smt_count = Statement.objects.filter(producer_element=self,
...
class System(auto_prefetch.Model, BaseModel):
    ...
    def set_fisma_impact_level(self, fisma_impact_level):
        ...
        smt, created = Statement.objects.get_or_create(statement_type="fisma_impact_level",
        ...
        for smt in smts:
            ...
            if smt.producer_element:
                ...
                smts_as_dict[smt.sid][
                    'combined_smt'] += f"<i>{smt.producer_element.name}</i><br/>{status_str}<br/><br/>{smt_formatted}<br/><br/>"
    ...
    def controls_status_count(self):
        ...
        counts = Statement.objects.filter(statement_type="control_implementation", status__in=status_list).values(
            'status').order_by('status').annotate(count=Count('status'))
        ...
        status_stats['Addressed'] = elm.statements_consumed.filter(statement_type="control_implementation").values(
            'sid').distinct().count()
        return status_stats
...
class CommonControlProvider(BaseModel):
