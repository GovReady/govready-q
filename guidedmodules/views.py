...
def download_module_output(request, task, answered, context, question, EncryptionProvider, set_ephemeral_encryption_cookies, document_id, download_format):
    ...
    if download_format == "html":
        ...
        # These are pandoc output formats. Except PDF, which is (via latex),
        # but can also more simply be generated by using -t html5 with a
        # filename with a .pdf extension if wkhtmltopdf is installed.
        # odt and some other formats cannot pipe to stdout, so we always
        # generate a temporary file. But note that these options work with
        # pandoc 1.16.0.2 and may not work with later versions that added
        # the pdf-engine option.
        pandoc_format, file_extension, mime_type = pandoc_opts[download_format]
        import tempfile, os.path, subprocess
        with tempfile.TemporaryDirectory() as tempdir:
            # convert from HTML to something else
            outfn = os.path.join(tempdir, document_id + "." + file_extension)
            subprocess.run(
                ["pandoc", "-f", "html", "-t", pandoc_format, "-o", outfn],
                input=doc["html"].encode("utf8"),
                check=True)
