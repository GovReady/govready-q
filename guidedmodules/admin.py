...
class AppSourceSpecWidget(forms.Widget):
    fields = [
        ...
        ("branch",
         "Branch",
         forms.TextInput(),
         "The name of the branch in the repository to read apps from. Leave blank to read "
         "from the default branch, which is usually 'master'.",
         {"git-web", "git-ssh"}),
        ("path",
         "Path",
         forms.TextInput(),
         "The path to the apps. For local directory AppSources, the local directory path. "
         "Otherwise the path within the repository, or blank if the apps are at the repository root.",
         {"local", "git-web", "git-ssh", "github"}),
        ("ssh_key",
         "SSH Key",
         forms.Textarea(attrs={"rows": 3}),
         "Paste an SSH private key here and send the public key to the repository owner. "
         "For GitHub and Gitlab repositories, add the public key as a deploy key in the repository's settings.",
         {"git-ssh"}),
        ("_remaining_",
         "Other Parameters",
...
class TaskAdmin(admin.ModelAdmin):
    list_display = ('id', 'title', 'organization_and_project', 'module',
                    'is_finished', 'submodule_of', 'editor', 'created',)
...
class TaskAnswerHistoryAdmin(admin.ModelAdmin):
    ...
    search_fields = ('taskanswer__task__project__organization__name',
                     'taskanswer__task__module__key', 'answered_by__username',
                     'answered_by__email')
    fieldsets = [
        (None, {"fields": ('created', 'taskanswer', 'answered_by')}),
