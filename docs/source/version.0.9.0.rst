...
Release 0.9.0 (coming Autumn 2019) is a minor release improving the user
...
Release 0.9.0 removes multi-tenancy and serves all pages from the same
...
faster. Release 0.9.0 begins to replace the “compliance app” terminology
...
For a complete list of changes see the `0.9.0.dev branch
CHANGELOG <https://github.com/GovReady/govready-q/blob/0.9.0.dev/CHANGELOG.md>`__.

Release 0.9.0 progress can be found on the ``0.9.0.dev`` and
``0.9.0.rc-xxx`` branches.
...
480px :alt: version 0.9.0 portfolio detail page
...
**Backup your database before upgrading to 0.9.0. Release 0.9.0 performs
database changes that makes rolling back difficult.**

See `Migration Guide for GovReady-Q (0.8.6 to
0.9.0) <migration_guide_086_09x.md>`__.

Installing 0.9.0
----------------

Release 0.9.0 simplifies the installation of GovReady-Q by removing the
need to manage subdomain-based multi-tenancy.

Three example assessments are preloaded during the install of 0.9.0 to
demonstrate the experience of using GovReady-Q.

The installation of assessments (aka “Compliance Apps”) requires an
extra step in 0.9.0 but greatly improves user experience.
...
.. _upgrading-to-0.9.0-from-0.8.x-1:
...
See `Migration Guide for GovReady-Q (0.8.6 to
0.9.0) <migration_guide_086_09x.html>`__.
